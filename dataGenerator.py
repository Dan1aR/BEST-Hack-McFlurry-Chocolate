...
#CONSTS
data_tosave_path = './data/noisySvin/'
ITERATOR = 0
iterFrames = 5
data_videos_path = './data/noisySvin/Выдосы нарезанные/'
...
def video_df_generator(data_train_path, video_file):
    global ITERATOR
    print(data_train_path + video_file)
    videocapture = cv2.VideoCapture(data_train_path + video_file)
    ...
    for i in range(iterFrames):
        ...
        xywhParams = detect_face(frame)
        if len(xywhParams) > 0:
            xIM, yIM, wIM, hIM = xywhParams
        else:
            continue
    ...
    if len(closing_s) > 0:    
        ...
        df = pd.DataFrame()
        df['delts'] = delts
        df.to_csv(data_train_path + video_file + '.csv') 
        ITERATOR += 1



for video_path in listdir(data_videos_path):
    for video_file in listdir(data_videos_path + video_path):
        data_train_path = data_videos_path + video_path + '/'
        video_df_generator(data_train_path, video_file)
